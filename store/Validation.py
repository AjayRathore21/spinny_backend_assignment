...
from spinny_backend_assignment.settings import A1, V1, L1, L2
...
def check_validity(user):
    ...
    boxes_last_week_by_user = Box.objects.filter(
        created_by=user, created_on__gt=datetime_one_week_ago
    ).count()
